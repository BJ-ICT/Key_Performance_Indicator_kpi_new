...
export default function FinalTables(){
  ...
  useEffect(()=>{
    ...
    if(row4 && row4.descriptionOfKPI){
      ...
      if(match4 && match4[1] && !isNaN(match4[1])){
        ...
        console.warn("Failed to extract threshold1 from Row #4's descriptionOfKPI");
      }
    }
  ...
  useEffect(() => {
    ...
    const row4 = kpiRes[3] ? rAchievedW(
      kpiRes[3].percentages['NW/WPC'],
      threshold4,
      parseFloat(kpiData.find(item => item.no === (kpiRes[3].no || kpiRes[3].rowNumber))?.weightage) || 0
    ) : 0;
  ...
  }, [kpiRes, subs, averagePlaceholder, servFulOkRow, columnSums, kpiData, threshold1, threshold2, threshold4]);
  ...
  const renderKpiRow = (kpiItem, rowIndex) => {
    ...
    if((rowIndex === 1 || rowIndex === 2 || rowIndex === 4) && threshold === null) {
      return null;
    }
...
const exportToExcel = () => {
  ...
  kpiRes.forEach((kpiItem, index) => {
    const threshold = index === 0 ? threshold1 : (index === 1 ? threshold2 : threshold4);
    const wpcVal = kpiItem.percentages["NW/WPC"] || "0.00";
    const wg = parseFloat(kpiData.find((item) => item.no === (kpiItem.no || kpiItem.rowNumber))?.weightage) || 0;
    // row 1, 2
...
dynamicTableRows[10] = [
  ...
  "",
  (() => {
    const targetRows = [1, 2, 4, 5, 6, 7, 10];
        ...
        <div className="final-table-container">
          ...
          <table className="final-distribution-table">
            ...
            <tbody>
              ...
              {/* 4) empty row */}
              {kpiRes[3] && renderKpiRow(kpiRes[3], 4)}
